...
-----------
    ...
    probe ready-record --output FILE
        --card-url URL [--card-proxy URL]
        --a2a-negative-url URL
        --a2a-send-url URL --a2a-token-file FILE --a2a-message TEXT [--a2a-proxy URL]
...
- Proxy routing is per-call and explicit (urllib.request.ProxyHandler +
  ...
  forgets --proxy cannot accidentally route inference (or anything else)
  through the Tailscale outbound path. A live-socket experiment (see the
  plan's Task 4 Step 4) confirmed the pinned Hermes/urllib stdlib path
  honors an explicit per-call HTTP proxy for an exact, even DNS-unresolvable,
...
def probe_card(url: str, proxy: Optional[str]) -> dict:
    try:
...
def probe_inference(base_url: str, model: str, token_file: str) -> dict:
    try:
        token = _read_token(token_file)
    except Exception as e:  # noqa: BLE001
    ...
    try:
        status, parsed = _http_post_json(url, body, proxy=None, headers=headers, timeout=60)
    except Exception as e:  # noqa: BLE001
        return {"step": "inference", "ok": False, "detail": f"{type(e).__name__}: {e}"}
...
def cmd_card(args: argparse.Namespace) -> int:
    return _print_and_exit(probe_card(args.url, args.proxy))
...
def cmd_a2a_send(args: argparse.Namespace) -> int:
    return _print_and_exit(probe_a2a_send(args.url, args.token_file, args.message, args.proxy))


def cmd_inference(args: argparse.Namespace) -> int:
    return _print_and_exit(probe_inference(args.base_url, args.model, args.token_file))


def cmd_ready_record(args: argparse.Namespace) -> int:
    results = [
        probe_card(args.card_url, args.card_proxy),
        probe_a2a_negative(args.a2a_negative_url),
        probe_a2a_send(args.a2a_send_url, args.a2a_token_file, args.a2a_message, args.a2a_proxy),
        probe_inference(args.inference_base_url, args.inference_model, args.inference_token_file),
    ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    card = sub.add_parser("card", help="fetch the A2A Agent Card, optionally through a proxy")
