...
def get_access_token():
    ...
    in the home directory, or if the refresh token is expired, 
    an authentication will be triggered.
    """
    ...
    # Get token
        ...
        if not os.path.isfile(TOKENS_PATH):
            raise FacilityAPIAuthError('Access token does not exist. '
                'Please authenticate by running "python3 alcf_facility_api_globus_token.py authenticate".')
